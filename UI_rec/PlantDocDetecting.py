...
from PlantDocDetection_UI import *
from PlantDoc.label_name import Chinese_name
...
# class PlantDoc_MainWindow(QBeautyUI):
class PlantDoc_MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, *args, obj=None, **kwargs):
        super(PlantDoc_MainWindow, self).__init__(*args, **kwargs)
    ...
    def slot_init(self):
        ...
        self.toolButton_video.clicked.connect(self.button_open_video_click) # 选择视频
        self.timer_video.timeout.connect(self.show_video)
        self.toolButton_camera.clicked.connect(self.button_open_camera_click)   # 选择摄像头
    ...
    def LoadModel(self, model_path=None):
        ...
        parser.add_argument('--weights', nargs='+', type=str, default='../weights/plant-best.pt',
                            help='model.pt path(s)')  # 模型路径仅支持.pt文件
        parser.add_argument('--img-size', type=int, default=320, help='inference size (pixels)')  # 检测图像大小
        parser.add_argument('--conf-thres', type=float, default=0.5, help='object confidence threshold')  # 置信度阈值
    ...
    def select_obj(self):
        ...
        if len(self.detInfo) > 0:
            for i, box in enumerate(self.detInfo):  # 遍历所有标记框
                ...
                # 画出检测到的目标物
                # self.names. box[0]
                image = self.drawRectBox(image, box[1],alpha=0.2, addText=label, color=self.colors[box[3]])
    ...
    def choose_file(self):
        ...
        fileName_choose, filetype=QFileDialog.getOpenFileName(None,"选取文件",os.getcwd(),"All Files(*)")
        ...
        if fileName_choose != '':
            ...
            if det is not None and len(det):  # 如果有检测信息则进入
                ...
                for *xyxy, conf, cls in reversed(det):  # 遍历检测信息
                    ...
                    # 画出检测到的目标物
                    # print(xyxy)
                    im0 = self.drawRectBox(im0, xyxy, alpha=0.2, addText=label, color=self.colors[int(cls)])  #mgchen
                    self.label_xmin_result.setText(str(c1[0]))
                    self.label_ymin_result.setText(str(c1[1]))
                    self.label_xmax_result.setText(str(c2[0]))
                    self.label_ymax_result.setText(str(c2[1]))
                    ...
                    res_all = [self.names[int(cls)], conf.item(), [c1[0], c1[1], c2[0], c2[1]]]
                    self.res_set.append(res_all)
                    # self.change_table(self.path, res_all[0], res_all[2], res_all[1])  #mgchen
    ...
    def button_open_video_click(self):
        ...
        if not self.timer_video.isActive():  # 检查定时状态
            # 弹出文件选择框选择视频文件
            fileName_choose, filetype = QFileDialog.getOpenFileName(None, "选取视频文件", os.getcwd(),
                                                                    "All Files(*)")
            ...
            if fileName_choose != '':
                ...
                # self.setStyleText(self.textEdit_video)    #mgchen
                ...
                for *xyxy, conf, cls in reversed(det):  # 遍历检测信息
                    ...
                    for cn in range(len(self.count_name)):
                        if self.names[int(cls)] == self.count_name[cn]:
                            count[cn] += 1
                    # 画出检测到的目标物
                    image = self.drawRectBox(image, xyxy,alpha=0.2, addText=label, color=self.colors[int(cls)])
                    self.label_xmin_result.setText(str(c1[0]))
                    self.label_ymin_result.setText(str(c1[1]))
                    self.label_xmax_result.setText(str(c2[0]))
                    self.label_ymax_result.setText(str(c2[1]))
    ...
    def button_open_camera_click(self):
        ...
        if not self.timer_camera.isActive():  # 检查定时状态
            ...
            if not flag:  # 相机打开失败提示
                ...
                #self.setStyleText(self.textEdit_camera)    #mgchen
                self.label_display.setText('正在启动识别系统...\n\nleading')
                QtWidgets.QApplication.processEvents()
    ...
    def display_image(self, image):
        # 显示结果到界面上
        show = cv2.resize(image, (804, 454))  # 格式转变，BGRtoRGB
    ...
    def drawRectBox(self, im0, xyxy, alpha, addText, color):
        if (isinstance(xyxy[0], int)):
            x0 = xyxy[0]
        else:
            x0 = xyxy[0].data.item()

        if (isinstance(xyxy[1], int)):
            y0 = xyxy[1]
        else:
            y0 = xyxy[1].data.item()

        if (isinstance(xyxy[2], int)):
            x1 = xyxy[2]
        else:
            x1 = xyxy[2].data.item()

        if (isinstance(xyxy[3], int)):
            y1 = xyxy[3]
        else:
            y1 = xyxy[3].data.item()

        cv2.rectangle(im0, (int(x0), int(y0)), (int(x1), int(y1)), color, 2)
        # 图片转换（cv2 -> pil）
        ...
        draw.text((x0, y0), addText, (100, 200, 200), font=font)

        # 图片转换（pil -> cv2）
        im0 = cv2.cvtColor(np.array(pilimg), cv2.COLOR_RGB2BGR)
        # cv2.putText(im0, addText, (x0, y0), cv2.FONT_HERSHEY_COMPLEX, 2.0, (100, 200, 200), 5)
        return im0

    # 加载背景图
    def load_backImg(self):
        im0 = cv2.imread("icons/back-image1.png")
        # 显示背景图片到界面
        show = cv2.resize(im0, (806, 454))
        show = cv2.cvtColor(show, cv2.COLOR_BGR2RGB)
